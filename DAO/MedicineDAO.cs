...
namespace QuanLyTiemThuoc.DAO
{
    public class MedicineDAO
    {
        ...
        public bool UpdateMedicine(int idMedicine, string name, int idCategory, float price)
        {
            string query = string.Format("update Medicine set name = N'{0}', idCategory = {1}, price = {2} where id = {3}", name, idCategory, price, idMedicine);
        ...
        {
            BillInforDAO.Instance.DeleteBillInforByMedicineID(idMedicine);
        ...
        public List<Medicine> SearchMedicineByName(string name)
        {
            ...
            string query = string.Format("exec USP_SearchMedicine N'%{0}%'", name);
